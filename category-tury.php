...
if (empty($rubric_banner_url)) {
	$rubric_banner_url = get_term_meta($term_id, 'rubric_banner_image_url', true);
			...
			<?php } ?>
			 ...
			 <h2 class="rubric-hotels-title">Популярные направления</h2>
			...
			<h2 class="rubric-hotels-title">Горящие туры по популярным направлениям</h2>
			<p>По популярным направлениям горящие туры появляются чаще благодаря большому выбору рейсов и отелей. Выберите страну и посмотрите актуальные предложения.</p>
			<div class="bg_block">
<?php 
